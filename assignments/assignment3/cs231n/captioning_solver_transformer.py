...
class CaptioningSolverTransformer(object):
    ...
    def __init__(self, model, data, idx_to_word, **kwargs):
        ...
        # Transformer 版本直接用 PyTorch Adam，而不是 numpy 风格的手写 optim.py。
    ...
    def _reset(self):
        ...
        manually.
        """
        # 这里只记录 epoch 和 loss 历史，结构比 numpy 版 solver 更轻量。
        self.epoch = 0
        self.loss_history = []
    ...
    def _step(self):
        ...
        be called manually.
        """
        # 随机取一个 batch 的图像特征和对应 caption。
        minibatch = sample_coco_minibatch(
            self.data, batch_size=self.batch_size, split="train"
        )
        captions, features, urls = minibatch

        # captions_in 喂给 decoder，当作输入前缀；captions_out 是右移一位后的监督目标。
        captions_in = captions[:, :-1]
        captions_out = captions[:, 1:]

        # mask 用来忽略 padding 的 <NULL> 位置，避免它们参与 loss。
        mask = captions_out != self.model._null

        # numpy 数据转成 torch tensor，供 PyTorch 模型前向与反向传播。
        ...
        # logits 形状是 (N, T, V)，表示每个时间步对全词表的打分。
        logits = self.model(t_features, t_captions_in)

        # temporal softmax 会在所有有效时间步上做交叉熵平均。
    ...
    def train(self):
        ...
        # iterations_per_epoch 和普通训练循环含义一样：每个 epoch 的 step 数。
        ...
        for t in range(num_iterations):
            ...
            # 这个布尔量通常可以扩展成保存 checkpoint / 调学习率之类的钩子。
