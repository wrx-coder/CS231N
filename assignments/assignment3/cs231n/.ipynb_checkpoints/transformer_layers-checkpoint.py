...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        #先把神经网络层的底层框架搭好，再写自己的内容

        #positional encoding 是一个网络层，可以像普通层一样放进模型里
        #继承自nn.module
        ...
        #unsqueeze(1) 的意思是：在第 1 维插入一个长度为 1 的维度，即变成列向量(max_len,1)
        div_term = torch.exp(torch.arrange(0, embed_dim, 2) * (-math.log(10000.0) / embed_dim))
        
        pe[0, :, 0::2] = torch.sin(position * div_term)
        #0：取 batch 维第 0 个，因为 pe 第一维本来就是 1   : 取所有位置  0::2：从第 0 维开始，每隔 2 个取一个
        ...
        #在 PyTorch 里，一个模块里大致有两类“重要张量”：1. Parameter
        #可学习参数，比如：
		#线性层权重
		#偏置，这些会被优化器更新。
        ...
        #2. Buffer
        #不是参数，但属于模型状态的一部分，比如：
    	#BatchNorm 的 running mean / running var
    	#这里的位置编码 pe
