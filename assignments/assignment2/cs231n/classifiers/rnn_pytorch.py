...
class CaptioningRNN:#caption：图片、表格下面的文字
    """
    ...
    def __init__(
        ...
        Inputs:
        - word_to_idx: A dictionary giving the vocabulary. 
        It contains V entries,
          and maps each string to a unique integer in the range [0, V).
          每个字符串(单词）映射到[0,v)的整数编号
        - input_dim: Dimension D of input image feature vectors.图像维度D
        - wordvec_dim: Dimension W of word vectors.词向量维度W
        ...
        self.cell_type = cell_type#rnn/lstm
        self.dtype = dtype#参数精度
    ...
    def loss(self, features, captions):#输入图片和正确句子，算loss
        """
        ...
        - loss: Scalar loss
        """
        # Cut captions into two pieces: 分成两半
        # captions_in has everything but the last word and will be input to the RNN; 
        # captions_out has everything but the first
    ...
    def sample(self, features, max_length=30):
       ...
       #1. 把当前词做 embedding
       #2. 做一步 RNN
       #3. 用 hidden state 算 vocab scores
       #4. 取分数最高的词作为 next word
       #5. 把 next word 写进 captions
       #6. 让 next word 成为下一轮输入
        N = features.shape[0]
